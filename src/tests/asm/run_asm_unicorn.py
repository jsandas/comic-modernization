...
for marker, val in patches:
    ...
    if idx == -1:
        ...
        continue
    print(f"[run_asm_unicorn.py] patching marker {marker} at {idx} -> value {val}", file=_sys.stderr)
    data[idx+len(marker)] = val & 0xFF
...
# Entry point for .com programs is at offset 0x100 (PSP header occupies first 256 bytes)
# Set segmented registers so the program entry (CS:IP) maps to ADDRESS + 0x100
entry_linear = ADDRESS + 0x100
...
try:
    ...
    if start_marker != -1:
        start_addr = ADDRESS + start_marker + len(b'ST')
    else:
        # fallback: typical .com entry at 0x100 offset
        start_addr = ADDRESS + 0x100
    uc.emu_start(start_addr, ADDRESS + len(data))
except Exception:
    pass
...
if args.set_tile is not None:
    ...
    if map_marker != -1:
        ...
        off = map_start + ty * 128 + tx
        try:
            memval = uc.mem_read(ADDRESS + off, 1)[0]
        ...
        try:
            debug_off = map_start + 1280
            dbg0 = uc.mem_read(ADDRESS + debug_off, 1)[0]
            dbg1 = uc.mem_read(ADDRESS + debug_off + 1, 1)[0]
            dbg2 = uc.mem_read(ADDRESS + debug_off + 2, 1)[0]
            dbg3 = uc.mem_read(ADDRESS + debug_off + 10, 1)[0]
            dbg4 = uc.mem_read(ADDRESS + debug_off + 11, 1)[0]
            print(f"[run_asm_unicorn.py] emu debug-slot @ {debug_off} = [{dbg0},{dbg1},{dbg2}] (AH/AL slots {dbg3},{dbg4})", file=_sys.stderr)
        except Exception as e:
