...
/**
 * Modify a paletted surface's palette entries to create a darkened version.
 * Specifically darkens entries 2, 10, and 12 to dark gray (0x18 in 6-bit EGA).
 * Returns true on success.
 */
static bool darken_palette_entries(SDL_Surface* surface) {
    if (!surface || !surface->format || !surface->format->palette) {
        std::cerr << "darken_palette_entries: surface is not paletted\n";
        return false;
    }

    SDL_Palette* pal = surface->format->palette;
    
    // EGA dark gray 0x18 (6-bit) → scaled to 8-bit RGB
    // Formula: 8-bit = (6-bit << 2) | (6-bit >> 4)
    constexpr uint8_t DARK_GRAY_6BIT = 0x18;
    const uint8_t dark_gray = (DARK_GRAY_6BIT << 2) | (DARK_GRAY_6BIT >> 4);

    // Palette entry indices to darken (from original DOS code)
    constexpr int PALETTE_REG_BACKGROUND = 2;   // Background color
    constexpr int PALETTE_REG_ITEMS = 10;       // Items/UI color
    constexpr int PALETTE_REG_TITLE = 12;       // Title text color

    if (PALETTE_REG_BACKGROUND < pal->ncolors) {
        pal->colors[PALETTE_REG_BACKGROUND].r = dark_gray;
        pal->colors[PALETTE_REG_BACKGROUND].g = dark_gray;
        pal->colors[PALETTE_REG_BACKGROUND].b = dark_gray;
    }

    if (PALETTE_REG_ITEMS < pal->ncolors) {
        pal->colors[PALETTE_REG_ITEMS].r = dark_gray;
        pal->colors[PALETTE_REG_ITEMS].g = dark_gray;
        pal->colors[PALETTE_REG_ITEMS].b = dark_gray;
    }

    if (PALETTE_REG_TITLE < pal->ncolors) {
        pal->colors[PALETTE_REG_TITLE].r = dark_gray;
        pal->colors[PALETTE_REG_TITLE].g = dark_gray;
        pal->colors[PALETTE_REG_TITLE].b = dark_gray;
    }
...
/**
 * Restore a paletted surface to its original palette.
 * (Reloads from the PNG file.)
 * Returns true on success.
 */
static bool restore_palette(SDL_Surface* surface, GraphicsSystem* graphics,
                            const char* filename) {
    std::string path = graphics->get_asset_path(filename);
    SDL_Surface* fresh = IMG_Load(path.c_str());
    if (!fresh || !fresh->format || !fresh->format->palette) {
        if (fresh) SDL_FreeSurface(fresh);
        return false;
    }

    // Copy the original palette over
    if (surface->format && surface->format->palette) {
        SDL_Palette* orig_pal = fresh->format->palette;
        SDL_Palette* target_pal = surface->format->palette;
        SDL_memcpy(target_pal->colors, orig_pal->colors,
                   sizeof(SDL_Color) * (orig_pal->ncolors < target_pal->ncolors
                                         ? orig_pal->ncolors : target_pal->ncolors));
    }
